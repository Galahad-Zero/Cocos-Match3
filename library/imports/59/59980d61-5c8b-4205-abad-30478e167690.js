...
// Script/gamePlay/common/EliminateUtils.ts
...
function getEliminateType(entry, eliminateBlockInfos) {
    ...
    if (rowCount >= 5) {
        // Base_5
        return GameTypes_1.EliminateType.BASE_5;
    }
    ...
    if (rowCount === 4) {
        if (columnCount >= 3) {
            // IRREGULAR_5
            return GameTypes_1.EliminateType.IRREGULAR_5;
        }
        else {
            // Base_4
            return GameTypes_1.EliminateType.BASE_4_ROW;
        }
    }
        ...
        if (rowCount >= 3) {
            ...
            // Base_4
            return GameTypes_1.EliminateType.BASE_4_COLUMN;
        }
    }
    // 基础3连消
    if (rowCount === 3) {
        if (columnCount >= 3) {
            ...
            // Base_3
            return GameTypes_1.EliminateType.BASE_3;
        }
    }
...
function generateSpecialBlock(blockConfig, eliminateType) {
    // 基础3连消不生成特殊方块
    if (eliminateType === GameTypes_1.EliminateType.BASE_3)
        return null;
...
function specialBlockGenerator(eliminateCheckInfo, gameCheckInfo) {
    // 如果是特殊消除类型,则不生成特殊方块
    if (eliminateCheckInfo.eliminateBlockType !== GameTypes_1.BlockSpecialType.NONE)
        return;
...
function releaseSpecialBlock(eliminateCheckInfo, alreadyEliminateBlocks, gameCheckInfo) {
    ...
    while (specialBlockConfig.length > 0) {
        ...
        if (specialBlockInfo.blockConfig.special === GameTypes_1.BlockSpecialType.ROW) {
            ...
            // 受影响的方块位置
            const effectedLocations = new Array(colCount)
                .fill(null)
                .map((_, index) => ({
                row: targetRow,
                column: index,
            ...
            // 受影响的方块位置
            const effectedLocations = new Array(rowCount)
                .fill(null)
                .map((_, index) => ({
                row: index,
                column: targetColumn,
        ...
        else if (specialBlockInfo.blockConfig.special ===
            GameTypes_1.BlockSpecialType.SPECIAL_BOOM) {
            // 同类消除
            ...
            for (let row = 0; row < gameCheckInfo.gameMapInfo.length; row++) {
                ...
                for (let col = 0; col < rowBlocks.length; col++) {
                    ...
                    if (block.type === targetType &&
                        block.special !== GameTypes_1.BlockSpecialType.SPECIAL_BOOM) {
                        effectLocations.push({
                            row,
                            column: col,
                        });
        ...
        const extraEliminateBlockInfos = extraEliminateLocations.reduce((acc, location) => {
            ...
            if (blockConfig.special !== GameTypes_1.BlockSpecialType.NONE) {
